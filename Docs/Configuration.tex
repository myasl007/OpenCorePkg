...
Most UEFI audio configuration is handled via the \hyperref[uefiaudioprops]{\texttt{UEFI Audio Properties}} section, but if
required the following additonal configuration options (which are known to be needed to produce sound on most Apple hardware)
may be specified in \texttt{UEFI/Drivers/Arguments}:
...
\begin{itemize}
  ...
  \tightlist
    ...
    set GPIO pin data high on specified pins. Required e.g. on MacBookPro10,2 and MacPro5,1.
    \item \texttt{0x00000002} (bit \texttt{1}) --- \texttt{GPIO\_SETUP\_STAGE\_DIRECTION},
    set GPIO data direction to output on specified pins. Required e.g. on MacPro5,1.
    \item \texttt{0x00000004} (bit \texttt{2}) --- \texttt{GPIO\_SETUP\_STAGE\_ENABLE},
    enable specified GPIO pins. Required e.g. on MacPro5,1. \medskip
    ...
    \emph{Note}: This option causes some problems with sound (previous sounds
    are not allowed to finish before new sounds start) on a small number of systems, therefore it
  ...
  \texttt{HDA: GPIO output enable 0x0F - Success} \medskip
